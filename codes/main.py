...
def evaluate_vae_anomaly_metrics_for_a_window(sess, model_vae, config, test_win):
...
def evaluate_lstm_anomaly_metric_for_a_seq(sess, model_vae, config, lstm_nn_model, test_seq):
...
def plot_histogram(config, test_anomaly_metric, n_bins, title, mean=None, std=None, xlim=None, saveplot=True):
    ...
    if saveplot:
        savefig(config['result_dir']+'reconstruction_error_histogram.png')
    # else:
    #     plt.show()
...
def plot_detected_anomalies(config, idx_detection, interval, dataset, result, detection_method, augmented_flag=1, y_scale=5, y_lim=None):
    ...
    axs.set_title("{} dataset test sequence\n(normalised by train mean {:.4f} and std {:.4f})\n Detection method: {}".format(dataset, 
                                                                                        ...
                                                                                        detection_method))
    axs.legend()
    savefig(config['result_dir']+'detected_anomalies_{}_aug_{}.png'.format(detection_method, augmented_flag))



def main():
    config = process_config('NAB_config.json')  # load VAE model
    create_dirs([config['result_dir'], config['checkpoint_dir']])  # create the experiments dirs
    # sess = tf.Session(config=tf.ConfigProto(log_device_placement=True))  # create tensorflow session
    sess = tf.Session()
    data = DataGenerator(config)
    model_vae = VAEmodel(config)  # create a CNN model
    trainer_vae = vaeTrainer(sess, model_vae, data, config)  # create a CNN model
    model_vae.load(sess)
    ...
    for i in range(n_val_vae):
        val_vae_recons_error[i], val_vae_kl_error[i], val_vae_elbo_loss[i] = evaluate_vae_anomaly_metrics_for_a_window(sess, model_vae, config, data.val_set_vae['data'][i])

    val_lstm_recons_error, val_lstm_embedding_error = np.zeros(n_val_lstm), np.zeros(n_val_lstm)
    for i in range(n_val_lstm):
        val_lstm_recons_error[i], val_lstm_embedding_error[i] = evaluate_lstm_anomaly_metric_for_a_seq(sess, model_vae, config, lstm_nn_model, data.val_set_lstm['data'][i])


    # Histogram of VAE ELBO loss - validation set
    vae_elbo_m, vae_elbo_std = plot_histogram(config, val_vae_elbo_loss, 100, 
    ...
    lstm_recons_m, lstm_recons_std = plot_histogram(config, val_lstm_recons_error, 100,  
    ...
    for i in range(n_test_lstm):
        test_lstm_recons_error[i], test_lstm_embedding_error[i] = evaluate_lstm_anomaly_metric_for_a_seq(sess, model_vae, config, lstm_nn_model, test_seq[i])
    ...
    _, _ = plot_histogram(config, test_lstm_recons_error, 100,
    ...
    plot_detected_anomalies(config,
                            idx_detection_augmented, 
